...
@Mapper
public interface YuyumhChapterMapper extends BaseMapper<YuyumhChapter> {
    ...
    @Select({
            ...
            "FROM yuyumh_chapter c",
            "LEFT JOIN (SELECT chapter_id, COUNT(*) AS pageCount FROM yuyumh_page GROUP BY chapter_id) p ON p.chapter_id = c.id",
